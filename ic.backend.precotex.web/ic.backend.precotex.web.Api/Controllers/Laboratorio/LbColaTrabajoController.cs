...
namespace ic.backend.precotex.web.Api.Controllers.Laboratorio
{
    ...
    public class LbColaTrabajoController : ControllerBase
    {
        ...
        */
        ...
        [HttpGet]
        [Route("getCargarDatosReporteTrico")]
        public async Task<IActionResult> getCargarDatosReporteTrico(string Corr_Carta, int Sec, string Tip_Ten)
        {
            var result = await _LbColaTrabajoService.CargarDatosReporteTrico(Corr_Carta, Sec, Tip_Ten);
            if (result!.Success)
            {
                result.CodeResult = StatusCodes.Status200OK;
                return Ok(result);
        ...
        [HttpGet]
        [Route("getCargarDatosReportePH")]
        public async Task<IActionResult> getCargarDatosReportePH(string Corr_Carta, int Sec, string Tip_Ten)
        {
            var result = await _LbColaTrabajoService.CargarDatosReportePH(Corr_Carta, Sec, Tip_Ten);
            if (result!.Success)
            {
                result.CodeResult = StatusCodes.Status200OK;
                return Ok(result);
        ...
        public async Task<IActionResult> getAnalisisDelta01_ObtieneDatosxPartida(string? sCod_OrdTra, string? sCod_Tela, string? sCod_Color, string? sText_Cod_Muestra, int? iStandardId, string? sCod_Usuario)
       {
        ...
        [HttpGet]
        [Route("getVerificarEstadoDispensado")]
        public async Task<IActionResult> getVerificarEstadoDispensado(string sTipo, string sCodUsuario)
        {
            var result = await _LbColaTrabajoService.VerificarEstadoDispensado(sTipo, sCodUsuario);
            if (result!.Success)
            {
                result.CodeResult = StatusCodes.Status200OK;
                return Ok(result);
