...
namespace ic.backend.precotex.web.Service.Services.Laboratorio
{
    public class LbColaTrabajoService : ILbColaTrabajoService
    {
        ...
        */
        ...
        public async Task<ServiceResponseList<Lb_Fijados_Tipo>?> ObtenerFijadosTipo()
        {
            var result = new ServiceResponseList<Lb_Fijados_Tipo>();
            try
            {
                var resultData = await _lbColaTrabajoRepository.ObtenerFijadosTipo();
                if (resultData == null || !resultData.Any())
                {
                    result.Success = true;
                    result.Message = "No existe información";
        ...
        public async Task<ServiceResponse<int>> ActualizarFijadoTipo(Lb_AgrOpc_Colorantes valores)
        {
            var result = new ServiceResponse<int>();
            try
            {
                var resultData = await _lbColaTrabajoRepository.ActualizarFijadoTipo(valores);
                if (resultData.Codigo > 0)
                {
                    result.Success = true;
                    result.Message = resultData.Mensaje;
        ...
        public async Task<ServiceResponse<int>> ActualizarEstadoCargaAhiba(Lb_Ahibas valores)
        {
            var result = new ServiceResponse<int>();
            try
            {
                var resultData = await _lbColaTrabajoRepository.ActualizarEstadoCargaAhiba(valores);
                if (resultData.Codigo > 0)
                {
                    result.Success = true;
                    result.Message = resultData.Mensaje;
        ...
        public async Task<ServiceResponse<int>> ActualizarPrevio(Lb_ColTra_Det valores)
        {
            var result = new ServiceResponse<int>();
            try
            {
                var resultData = await _lbColaTrabajoRepository.ActualizarPrevio(valores);
                if (resultData.Codigo > 0)
                {
                    result.Success = true;
                    result.Message = resultData.Mensaje;
        ...
        public async Task<ServiceResponseList<Reporte_ph>?> CargarDatosReportePH(string Corr_Carta, int Sec, string Tip_Ten)
        {
        ...
        public async Task<ServiceResponse<int>> ValidarCorridaDuplicadaAsync(string Corr_Carta, int Sec, int Correlativo, string Tip_Receta, string Usr_Cod)
        {
            var result = new ServiceResponse<int>();

            try
            {
                var resultData = await _lbColaTrabajoRepository.ValidarCorridaDuplicadaAsync(Corr_Carta, Sec, Correlativo, Tip_Receta, Usr_Cod);
                if (resultData.Codigo > 0)
                {
                    result.Success = true;
                    result.Message = resultData.Mensaje;
        ...
        public async Task<ServiceResponse<int>> EntregarCorridaAsync(string Corr_Carta, int Sec, int Correlativo, string Procedencia, string Tip_Ten, string Usr_Cod)
        {
            var result = new ServiceResponse<int>();

            try
            {
                var resultData = await _lbColaTrabajoRepository.EntregarCorridaAsync(Corr_Carta, Sec, Correlativo, Procedencia, Tip_Ten, Usr_Cod);
                if (resultData.Codigo > 0)
                {
                    result.Success = true;
                    result.Message = resultData.Mensaje;
